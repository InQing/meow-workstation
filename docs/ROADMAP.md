...
| # | 问题                                                                                            | 影响                                                  | 修法                                     |
| - | --------------------------------------------------------------------------------------------- | --------------------------------------------------- | -------------------------------------- |
| 1 | **`DEFAULT_SAVE` 与运行时键不同步** —— `settings.petScale`、`stats.bestReflexMs`、`stats.reflexBreaks` 是功能写入的，没进默认档 | 靠 `mergeDeep` 兜住，能跑；但默认值语义不清，`save.js` 读起来会漏        | 补进 `src/main/save.js` 的 `DEFAULT_SAVE` |
