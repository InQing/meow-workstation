...
| 点一下猫       | 摸头：+2 好感，8 秒冷却（专注中只劝你干活，见番茄钟） |
...
**番茄钟** — 猫右侧有个沙漏：**上半的沙就是剩余时间**，漏完即到点。沙漏是木质玻璃沙漏配一圈花环（矢量描绘，任意缩放都清晰），专注时金砂、休息时换成蓝砂、暂停时整只褪成墨绿单色版并静止，小字显示「暂停」（能分清"暂停"和"这局结束了"）。专注 25 分钟 → 自动进 5 分钟休息，到点弹系统通知。完成一个番茄：**+30 金币、+1 抽卡券、+10 好感**。时长在面板「设置」里改。**专注期间不乱冒泡**：闲聊、游戏反应都静默，只有沙漏安静陪工；主动摸头 / 喂食会换来一句「劝你专心」（不给好感，喂食也不扣金币）。同一个番茄里烦它超过 5 次后，每次都会「……」并扣 1 好感；这个番茄的结算还会打折 —— **金币 30→15、不送券、完成好感 10→5**，完成提示也会明说「摸鱼太多」。暂停 / 继续 / 番茄完成的提示照常。

**在场感知** — 猫知道你在不在工位：离开 2 分钟后它安静下来（不冒泡、不自己玩），离开 15 分钟会自己打盹；你回来时伸懒腰打招呼，离开久了还会抱怨「鱼干都放凉了」。专注中回来另说一句「终于回来啦，继续工作吧」。锁屏 / 休眠直接按离开算；阈值在 `src/shared/config.js` 的 `presence` 段。
...
src/
  shared/               纯逻辑层（UMD，node 可 require 自测）
    ...
    presence.js         在场状态纯逻辑：空闲秒数 → active/away/sleeping、变化检测
  ...
  pet/                  桌宠窗口
    ...
    hourglassArt.js     番茄沙漏矢量几何 + 三态配色（由 resources/沙漏花环-*.svg 提取生成）
...
npm test           # 七个纯逻辑层自测（不需要 electron）
```
