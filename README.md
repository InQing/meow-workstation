...
**番茄钟** — 猫左侧有个沙漏：**上半的沙就是剩余时间**，漏完即到点。专注 25 分钟 → 自动进 5 分钟休息，到点弹系统通知。完成一个番茄：**+30 金币、+1 抽卡券、+10 好感**。暂停时沙漏灰掉静止、小字显示「暂停」（能分清"暂停"和"这局结束了"）。时长在面板「设置」里改。**专注期间不乱冒泡**：闲聊、游戏反应都静默，只有沙漏安静陪工；主动摸头 / 喂食会换来一句「劝你专心」（不给好感，喂食也不扣金币）。同一个番茄里烦它超过 5 次后，每次都会「……」并扣 1 好感；这个番茄的结算还会打折 —— **金币 30→15、不送券、完成好感 10→5**，完成提示也会明说「摸鱼太多」。暂停 / 继续 / 番茄完成的提示照常。
